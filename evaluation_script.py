...
filename = sys.argv[2]
out_path = sys.argv[1][:-4] + "_scores.txt"


with open(filename,"rt",errors="ignore",encoding="utf-8") as data_file:    
	Goldstandard = csv.reader(data_file, delimiter=',')
	for gold in Goldstandard:
		if len(gold[0]) > 0 and len(gold[2]) > 0:
			gold[0] = gold[0].replace("_","")
			golddict[gold[0]] = int(gold[2])
with open(inputfilename,"rt",errors="ignore",encoding="utf-8") as data_file2:    
...
file = open(out_path, "w")
file.write("F1-SCORE: " + str(finsco) + "\n")
file.write('Accuracy: ' + str(accuracy_score(golddictlist,inputdictlist)*f1weight) + "\n")
file.write('Precision: '+ str(precision_score(golddictlist,inputdictlist)*f1weight) + "\n")
file.write('Recall: ' + str(recall_score(golddictlist,inputdictlist)*f1weight) + "\n")
file.close()